...
must_haves:
  truths:
    - "GitHubConnection model exists in database schema"
    - "Project model has GitHub sync fields (githubFullName, isGitHubSynced, customizedFields, lastSyncedAt)"
    - "Octokit client can be instantiated with an access token"
    - "GitHub access tokens can be encrypted and decrypted"
  artifacts:
    - path: "prisma/schema.prisma"
      provides: "GitHubConnection model and Project extensions"
...
3. Extend Project model with GitHub sync fields (add to existing model):
   ```prisma
...
- Project table has new columns visible in Prisma Studio
  </verify>
  <done>GitHubConnection model exists, Project model has githubFullName, isGitHubSynced, customizedFields, lastSyncedAt fields</done>
</task>
...
- Project model extended with githubFullName, isGitHubSynced, customizedFields, lastSyncedAt
