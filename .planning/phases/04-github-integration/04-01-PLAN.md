...
must_haves:
  truths:
    ...
    - "Octokit client initializes with a token and can be used for API calls"
    - "Token encryption roundtrip works: encrypt then decrypt returns original value"
    - "GitHubConnection table is queryable in Prisma Studio"
  artifacts:
...
3. Add GitHub sync tracking fields to existing Project model.
   ...
   Add these NEW fields to the existing Project model:
   ```prisma
...
- Project table has new columns visible in Prisma Studio (githubFullName, isGitHubSynced, customizedFields, lastSyncedAt)
  ...
  <done>GitHubConnection model exists, Project model has githubFullName, isGitHubSynced, customizedFields, lastSyncedAt fields (githubId, stars, forks, language already existed)</done>
</task>
